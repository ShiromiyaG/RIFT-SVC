...
# Files generated by prepare_pitch_aug.py; they only ever go to the train split
AUG_MARKER = '_pshift'
...
def generate_meta_info(speaker_to_files, split_type, num_test, num_test_per_speaker, seed, only_include_speakers=None):
    ...
    # Split only the original files; pitch-augmented copies are appended to the
    # train set afterwards so they can never land in the test set
    aug_audios = []
    original_files = {}
    for speaker, files in speaker_to_files.items():
        originals = [f for f in files if AUG_MARKER not in f]
        aug_audios.extend(
            {"speaker": speaker, "file_name": f} for f in files if AUG_MARKER in f
        )
        if originals:
            original_files[speaker] = originals

    if split_type == 'random':
        train_audios, test_audios = perform_random_split(original_files, num_test, seed)
    elif split_type == 'stratified':
        train_audios, test_audios = perform_stratified_split(original_files, num_test_per_speaker, seed, only_include_speakers)
    else:
        raise ValueError("Invalid split_type. Choose 'random' or 'stratified'.")

    train_audios = train_audios + aug_audios
